...
# This script runs the complete LLM-based data analysis application
# Features: FastAPI backend + React frontend + AI-powered CSV analysis
...
function Kill-ProcessOnPort {
    ...
    try {
        ...
        foreach ($pid in $processes) {
            ...
            Write-ColorOutput "   Killed process on port $Port" $Yellow
        }
    }
    catch {
...
function Start-Frontend {
    ...
    Start-Sleep -Seconds 15  # Give frontend time to start
    
    # Check multiple possible ports (Vite default + common alternatives)
    $portsToCheck = @($frontendPort, 8080, 8081, 5173, 3000, 3001)
...
function Check-ProjectStatus {
    ...
    # Check frontend
    $frontendFound = $false
    $portsToCheck = @(8080, 8081, 5173, 3000, 3001)
...
Write-ColorOutput "║                POLARIS DATA WHISPERER                        ║" $Magenta
Write-ColorOutput "║              LLM-Based Data Analysis App                     ║" $Magenta
Write-ColorOutput "╚══════════════════════════════════════════════════════════════╝" $Magenta
Write-Host ""

# Kill existing processes if requested
if ($KillExisting) {
    Write-ColorOutput "🔄 Killing existing processes..." $Yellow
    Kill-ProcessOnPort 8000  # Backend
    Kill-ProcessOnPort 8080  # Frontend (Vite default)
    Kill-ProcessOnPort 8081  # Frontend (alternative)
    Kill-ProcessOnPort 5173  # Frontend (Vite default)
...
if (-not (Test-Path "node_modules")) {
    ...
    Write-ColorOutput "   Example: .\run-project-final.ps1 -InstallDeps" $Cyan
    Write-Host ""
}
...
# Set ports
$backendPort = 8000
$frontendPort = Find-AvailablePort 8080  # Vite default port

# Check if ports are available
if (Test-Port $backendPort) {
    Write-ColorOutput "⚠️  Port $backendPort is already in use (backend)" $Yellow
    if (-not $KillExisting) {
        Write-ColorOutput "   Use -KillExisting flag to kill existing processes" $Cyan
    }
}
...
try {
    ...
    Write-ColorOutput "   • AI-powered CSV analysis using Google Gemini" $White
    Write-ColorOutput "   • Natural language to SQL conversion" $White
    Write-ColorOutput "   • Real-time data insights and visualizations" $White
    Write-ColorOutput "   • Modern React UI with Tailwind CSS" $White
    Write-Host ""
